...
The tool aims to support fast yet informed decision making a 'first pass' to help focus planning attention on road sections that are most likely to be suitable for new schemes.

The rapid cycleway prioritisation tool aims to help produce evidence-based answers the question: **where to reallocate road space on strategic corridors away from motor traffic to enable uptake of cycling?**
...
Road space reallocation is suited to larger roads and can have a large impact quickly, especially when moving large numbers of people away from public transport and capacity is a priority.
...
The numbers represents the number of one way cycle trips to work and school that could be expected when the [Government's aim of doubling cycling is met](https://www.gov.uk/government/publications/cycling-and-walking-investment-strategy).
...
Road sections that are on strategic and major road networks (SRN and MRN respectively) and which had a high speed limit (40 mph+) before the COVID-19 crises can be in the top routes.
...
This layer is represents what a joined-up cycle network could look like if new cycleways were placed along corridors of high potential, by either closing roads to motorised traffic or creating one-way systems.
...
- The results are based on the existing road network and does not flag potential routes that could be constructed, e.g. along old railway tracks or across open areas.
...
- Other than the Cohesive network layer, which highlights what a joined-up cycle network could look like if roads could be made one way to motor traffic and opened-up to cycling via modal filters, the results do not provide evidence on the full range of interventions. Identifying locations for filtered permeability interventions in residential areas, for example, requires data on residential networks not contained in this analysis, for example.
