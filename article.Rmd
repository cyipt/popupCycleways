...
r_cleaned = r_original %>% 
  ...
  mutate(pctgov = as.numeric(pctgov)) 
table(r_cleaned$maxspeed)
rj = inner_join(r_cleaned, rtid) %>% 
  mutate(lanes_f = lanespsvforward + lanesforward) %>% 
  mutate(lanes_b = lanespsvbackward + lanesbackward) %>% 
  mutate(`N. lanes` = lanes_f + lanes_b)
...
knitr::include_graphics("https://user-images.githubusercontent.com/1825120/81476361-8c094480-9209-11ea-97d4-f4ad09e96d69.png")
```

```{r hospitals, fig.cap="Overview map of input data, showing the main highway types and location of hospitals in Leeds"}
m1 = r_main_region %>%
  mutate(`Highway type` = highway2) %>% 
  mutate(`Cycling potential` = case_when(pctgov < 100 ~ 100, TRUE ~ pctgov)) %>% 
...
m1 = tm_shape(r_main_region) +
  tm_lines("maxspeed", palette = "-Set1", lwd = "N. lanes", scale = 5,
           lwd.legend = c(1, 2, 3, 4, 5, 6)) +
