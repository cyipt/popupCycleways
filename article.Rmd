...
There has been a notable increase in active travel, particularly cycling, in some areas  [@harrabin_boom_2020].
...
We set a modifiable parameter `city_centre_buffer_radius` with an initial value of 8 km (5 miles) to geographically subset potential routes. This represents a distance that most people have the physical ability to cycle.
...
r_pct_top_n = r_pct_top %>% 
  group_by(name) %>% 
  slice(which.max(cycling_potential)) %>% 
  filter(name != "") %>% 
  ungroup() %>% 
  top_n(10, wt = km_day) 
...
knitr::include_graphics("figures/results-leeds.png") # this needs changing
```

```{r}
res_table = r_pct_top_n %>% 
  sf::st_drop_geometry() %>% 
  arrange(desc(cycling_potential)) %>% 
