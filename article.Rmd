...
Road segments with a reference number were grouped together with neighbours within a 100 m buffer, using the `igraph` R package. 
...
For all segments, a novel round of grouping (ignoring previous groups) with a 100 m buffer was then used to remove groups with length below 500 m. 
...
m2 = tm_shape(city_centre_buffer) + tm_borders(col = "grey") +
  tm_shape(r_lanes_joined) + tm_lines("group_id", palette = "Dark2") +
